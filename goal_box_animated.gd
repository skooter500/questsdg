...
func bounce_in():
	if fade_tween:
		...
		var v0:VideoStream = load("res://goals/videos/" + str(goal) + "_SDG_MakeEveryDayCount_Gifs_GDU.ogv")
		$SubViewport/VideoStreamPlayer.stream = v0
		$SubViewport/VideoStreamPlayer.play()
		
		var v1:VideoStream = load("res://goals/videos/E_GIF_" + ("%02d" % goal) + ".ogv")
		$SubViewport2/VideoStreamPlayer.stream = v1
		
		$SubViewport2/VideoStreamPlayer.play()
		scale = Vector3.ZERO
