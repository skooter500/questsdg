...
# step = 0..2 (pipe connected, then button pressed)
func set_progress(step: int, total: int = 2) -> void:
	var clamped_step: int = clampi(step, 0, cells.size())

	if progress_label:
		progress_label.text = "Geothermal Plant Setup: %d / %d" % [clamped_step, total]
